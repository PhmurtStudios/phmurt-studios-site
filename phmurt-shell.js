(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── ONBOARDING: Guided first-visit walkthrough (homepage only) ───
    if (window.location.pathname === '/' || window.location.pathname.indexOf('index.html') !== -1) {
      setTimeout(function () { setupOnboarding(); }, 800);
    }
  });

  /* ═══════════════════════════════════════════════════════════
     ONBOARDING WALKTHROUGH — shows on first visit to homepage
     ═══════════════════════════════════════════════════════════ */
  function setupOnboarding() {
    try {
      if (localStorage.getItem('phmurt_onboarded') === 'true') return;
    } catch (e) { return; }

    // Don't show if user is already signed in (returning user)
    if (window.PhmurtDB && PhmurtDB.getSession && PhmurtDB.getSession()) return;

    var steps = [
      { title: 'Welcome, Adventurer!', text: 'Phmurt Studios is your free D&D homebrew toolkit — character builders, generators, campaigns, and a full content library.', icon: '\u2694\uFE0F' },
      { title: 'Build a Character', text: 'Jump into the Character Builder to create a full D&D 5e character step by step, or grab one of 40 premade characters from the Gallery.', icon: '\uD83D\uDCDC', link: 'gallery.html', linkText: 'Browse Gallery' },
      { title: 'Generate Anything', text: 'Need an NPC, tavern, encounter, or quest on the fly? Our random generators have you covered.', icon: '\uD83C\uDFB2', link: 'generators.html', linkText: 'Open Generators' },
      { title: 'Create an Account', text: 'Sign up free to save characters, run campaigns, and share your homebrew creations with the community.', icon: '\u2728', action: 'auth' }
    ];
    ...
    overlay.id = 'ps-onboard-overlay';
    overlay.innerHTML =
      '<div class="ps-onboard-backdrop"></div>' +
      '<div class="ps-onboard-card">' +
        '<button class="ps-onboard-close" aria-label="Close">\u2715</button>' +
        '<div class="ps-onboard-icon" id="ps-onboard-icon"></div>' +
        '<div class="ps-onboard-title" id="ps-onboard-title"></div>' +
        '<div class="ps-onboard-text" id="ps-onboard-text"></div>' +
        '<div class="ps-onboard-link" id="ps-onboard-link"></div>' +
        '<div class="ps-onboard-dots" id="ps-onboard-dots"></div>' +
        '<div class="ps-onboard-btns">' +
          '<button class="ps-onboard-skip" id="ps-onboard-skip">Skip</button>' +
          '<button class="ps-onboard-next" id="ps-onboard-next">Next</button>' +
        '</div>' +
      '</div>';
    ...
    style.textContent =
      '#ps-onboard-overlay{position:fixed;inset:0;z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeInUp .4s ease;}' +
      '.ps-onboard-backdrop{position:absolute;inset:0;background:rgba(0,0,0,0.75);}' +
      '.ps-onboard-card{position:relative;background:var(--bg-card,#141420);border:1px solid var(--crimson-border,rgba(212,67,58,0.36));max-width:440px;width:100%;padding:40px 36px 32px;text-align:center;z-index:1;}' +
      '.ps-onboard-card::before{content:"";position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,var(--crimson,#d4433a),transparent);}' +
      '.ps-onboard-close{position:absolute;top:12px;right:16px;background:none;border:none;color:var(--text-muted,#9a8870);font-size:16px;cursor:pointer;padding:8px;min-width:44px;min-height:44px;}' +
      '.ps-onboard-close:hover{color:var(--text,#f2e8d6);}' +
      '.ps-onboard-icon{font-size:40px;margin-bottom:16px;opacity:0.9;}' +
      '.ps-onboard-title{font-family:"Cinzel",serif;font-size:20px;letter-spacing:2px;color:var(--text,#f2e8d6);margin-bottom:14px;}' +
      '.ps-onboard-text{font-family:"Spectral",serif;font-size:15px;color:var(--text-dim,#cfc0a8);line-height:1.75;margin-bottom:18px;}' +
      '.ps-onboard-link{margin-bottom:16px;}' +
      '.ps-onboard-link a{font-family:"Cinzel",serif;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--crimson,#d4433a);border:1px solid var(--crimson-border,rgba(212,67,58,0.36));padding:8px 18px;display:inline-block;transition:all .2s;}' +
      '.ps-onboard-link a:hover{background:var(--crimson-soft,rgba(212,67,58,0.14));color:var(--text,#f2e8d6);}' +
      '.ps-onboard-dots{display:flex;gap:8px;justify-content:center;margin-bottom:20px;}' +
      '.ps-onboard-dot{width:8px;height:8px;border-radius:50%;background:var(--border,rgba(255,255,255,0.11));transition:all .2s;}' +
      '.ps-onboard-dot.active{background:var(--crimson,#d4433a);transform:scale(1.2);}' +
      '.ps-onboard-btns{display:flex;gap:12px;justify-content:center;}' +
      '.ps-onboard-skip{font-family:"Cinzel",serif;font-size:10px;letter-spacing:2px;text-transform:uppercase;background:none;border:1px solid var(--border,rgba(255,255,255,0.11));color:var(--text-muted,#9a8870);padding:10px 20px;cursor:pointer;transition:all .2s;min-height:44px;}' +
      '.ps-onboard-skip:hover{border-color:var(--text-muted);color:var(--text-dim);}' +
      '.ps-onboard-next{font-family:"Cinzel",serif;font-size:10px;letter-spacing:2px;text-transform:uppercase;background:var(--crimson,#d4433a);border:none;color:#f5f0e8;padding:10px 24px;cursor:pointer;transition:all .2s;min-height:44px;}' +
      '.ps-onboard-next:hover{background:var(--crimson-dim,#a82a20);}' +
      '@media(max-width:480px){.ps-onboard-card{padding:32px 24px 28px;}.ps-onboard-title{font-size:18px;}.ps-onboard-text{font-size:14px;}}';
    document.head.appendChild(style);

    function showStep(i) {
      var s = steps[i];
      document.getElementById('ps-onboard-icon').textContent = s.icon;
      document.getElementById('ps-onboard-title').textContent = s.title;
      document.getElementById('ps-onboard-text').textContent = s.text;

      var linkEl = document.getElementById('ps-onboard-link');
      if (s.link) {
        linkEl.innerHTML = '<a href="' + s.link + '">' + s.linkText + ' \u2192</a>';
      } else {
        linkEl.innerHTML = '';
      }

      // Dots
      var dotsEl = document.getElementById('ps-onboard-dots');
      dotsEl.innerHTML = '';
      for (var d = 0; d < steps.length; d++) {
        var dot = document.createElement('div');
        dot.className = 'ps-onboard-dot' + (d === i ? ' active' : '');
        dotsEl.appendChild(dot);
      }

      // Button text
      var nextBtn = document.getElementById('ps-onboard-next');
      if (i === steps.length - 1) {
        if (s.action === 'auth') {
          nextBtn.textContent = 'Sign Up Free';
        } else {
    ...
    function dismiss() {
      ...
      try { localStorage.setItem('phmurt_onboarded', 'true'); } catch (e) {}
    }

    overlay.querySelector('.ps-onboard-backdrop').addEventListener('click', dismiss);
    overlay.querySelector('.ps-onboard-close').addEventListener('click', dismiss);
    document.getElementById('ps-onboard-skip') || overlay.querySelector('.ps-onboard-skip');
    overlay.querySelector('.ps-onboard-skip').addEventListener('click', dismiss);

    overlay.querySelector('.ps-onboard-next').addEventListener('click', function () {
      if (currentStep < steps.length - 1) {
        ...
        if (lastStep.action === 'auth' && window.PhmurtDB && PhmurtDB.openAuth) {
          PhmurtDB.openAuth();
        }
      }
    });
